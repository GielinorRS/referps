...
public class MorningstarCombatScript implements CombatStrategy {
    ...
    public void regularAttack(Character victim, NPC solak) {
        ...
        solak.setChargingAttack(true);
        solak.getCombatBuilder().setContainer(new CombatContainer(solak, victim, 1, 1, CombatType.MELEE, true));

        TaskManager.submit(new Task(1, solak, false) {
            int tick = 0;

            @Override
            public void execute() {
                if (tick == 2) {
                    solak.setChargingAttack(false);
                    stop();
                }
                tick++;
            }
            ...
            public void execute() {// every 2 ticks for 5 seconds
                if (tick < 10 && tick % 2 != 0) {
                    ...
                    for (Player t : Misc.getCombinedPlayerList(target)) {
                        ...
                        t.dealDamage(new Hit(300, Hitmask.RED, CombatIcon.MELEE));
